...
document.addEventListener("turbo:load", function () {
  ...
  let getAttendeeTotals = function () {
    ...
    $("input#event_registration_registration_fee").val(
      attendeeTotal.toFixed(2),
    );

    // Update counts for backend processing
    $("input#event_registration_number_of_adults").val(
      $('#attendees input[value="adult"]:visible:checked').length,
    );
    $("input#event_registration_number_of_youths").val(
      $('#attendees input[value="youth"]:visible:checked').length,
    );
    $("input#event_registration_number_of_children").val(
      $('#attendees input[value="child"]:visible:checked').length,
    );
  ...
  let setTotal = function () {
    ...
    if (document.documentElement.hasAttribute("data-turbo-visit_control"))
      return;
    ...
    if (typeof appData !== "undefined") {
      let regFee =
        parseFloat($("input#event_registration_registration_fee").val()) ||
        parseFloat(appData.event_registration_fee);
      let donation =
        parseFloat($('input[name="event_registration[donation]"]').val()) || 0;
  ...
  $firstAttendee
    .find('input[value="youth"], input[value="child"]')
    .prop("disabled", true)
    .parent()
    .hide();
  $firstAttendee
    .find(".event_registration_attendees_name input")
