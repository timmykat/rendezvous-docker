...
export const initAutocomplete = () => {
  ...
  const newRegistrationPage = document.querySelector(
    "body.c_event_registrations .new_registration",
  );
  ...
  if (donationPage || newRegistrationPage) {
    ...
    const targetFields = autoComplete.querySelectorAll(
      "[data-autocomplete-target]",
    );
    const firstAttendeeField = document.querySelector(
      'input[name="event_registration[attendees_attributes][0][name]"',
    );
    const autocompleteTrigger = autoComplete.querySelector(
      ...
      fetch(url, { headers: getCsrfHeaders() })
        ...
        .then((data) => {
          ...
          if (data.existing_registration) {
            alert(
              `This user already has a registration! (${data.existing_registration})`,
            );
            return;
          }
          targetFields.forEach((field) => {
